...
def _positions_for_profile(profile: ResolvedAgentProfile) -> Array:
    ...
    return jnp.where(profile.active_mask[:, None], positions, 0.0)
...
@pytest.mark.parametrize(
    ...
    (
        (cast(Any, []), TypeError, "initial_agent_positions"),
        (
            jnp.zeros((MAX_AGENT_SLOTS - 1, ENVIRONMENT_DIMENSIONS), dtype=jnp.float32),
            ValueError,
            "shape",
        ),
        (
            jnp.zeros((MAX_AGENT_SLOTS, ENVIRONMENT_DIMENSIONS), dtype=jnp.int32),
            TypeError,
            "dtype",
        ),
    ),
)
def test_initial_position_storage_contract_is_enforced(
    invalid_positions: object, error_type: type[Exception], message: str
) -> None:
    with pytest.raises(error_type, match=message):
        validate_env_config(
            _replace_config(_valid_config(), initial_agent_positions=invalid_positions)
        )


def test_initial_positions_reject_nonfinite_inactive_bounds_and_body_overlap() -> None:
    config = _valid_config(team_sizes=(2, 1))
    positions = config.initial_agent_positions
    invalid_cases = (
        (positions.at[0, 0].set(jnp.nan), "finite"),
        (positions.at[2].set(jnp.asarray((1.0, 1.0))), "inactive"),
        (positions.at[0, 0].set(0.49), "bounds"),
        (positions.at[1].set(positions[0]), "bodies overlap"),
    )
    for invalid_positions, message in invalid_cases:
        with pytest.raises(ValueError, match=message):
            validate_env_config(
                config._replace(initial_agent_positions=invalid_positions)
            )


def test_initial_positions_reject_pillar_and_rotated_wall_overlap() -> None:
    pillar_config = _valid_config(
        team_sizes=(1, 0),
        obstacles=_empty_obstacles().at[0].set(_pillar(x=2.0, y=2.0)),
    )
    pillar_positions = pillar_config.initial_agent_positions.at[0].set(
        jnp.asarray((2.0, 2.0), dtype=jnp.float32)
    )
    with pytest.raises(ValueError, match="pillar"):
        validate_env_config(
            pillar_config._replace(initial_agent_positions=pillar_positions)
        )
    ...
    wall_positions = wall_config.initial_agent_positions.at[0].set(
        jnp.asarray((8.0, 4.0), dtype=jnp.float32)
    )
    with pytest.raises(ValueError, match="wall"):
        validate_env_config(
            wall_config._replace(initial_agent_positions=wall_positions)
        )
...
def test_extreme_finite_pillar_overlap_uses_overflow_safe_distance() -> None:
    ...
    positions = config.initial_agent_positions.at[0].set(
...
def test_bounds_pillar_wall_and_agent_tangency_are_legal() -> None:
    ...
    positions = jnp.zeros((MAX_AGENT_SLOTS, 2), dtype=jnp.float32)
    positions = positions.at[0].set(jnp.asarray((0.5, 0.5), dtype=jnp.float32))
    positions = positions.at[1].set(jnp.asarray((1.5, 0.5), dtype=jnp.float32))
    config = EnvConfig(
        max_steps=10,
        map_width=12.0,
        map_height=8.0,
    ...
    assert (
        validate_env_config(config._replace(initial_agent_positions=pillar_tangent))
        is None
    )
    ...
    assert (
        validate_env_config(config._replace(initial_agent_positions=wall_tangent))
        is None
    )
...
def test_pillar_validation_matches_public_m4_projection(
    ...
    config = base_config._replace(
        initial_agent_positions=base_config.initial_agent_positions.at[0].set(center)
    )
...
def test_distant_rotated_wall_clearance_ignores_projection_roundoff(
    ...
    config = config._replace(
        initial_agent_positions=config.initial_agent_positions.at[0].set(agent_center)
    )
...
def test_reset_is_key_independent_jittable_and_stay_needs_no_repair() -> None:
    ...
    assert bool(
        jnp.array_equal(first[0].agent_positions, config.initial_agent_positions)
    )
    ...
    assert bool(
        jnp.array_equal(next_state.agent_positions, config.initial_agent_positions)
    )
