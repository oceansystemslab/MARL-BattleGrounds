...
def _direct_fields(kind: str) -> tuple[str, ...]:
    return {
        ...
        "team_deathmatch_completed": ("outcome", "completion_basis"),
    }[kind]


def test_all_24_events_project_one_to_one_without_internal_slot_axes() -> None:
    batch, _ = _all_event_batch()
    ...
    assert summary.event_count == 24
    assert len({type(event) for event in summary.events}) == 24
    ...
    assert tuple(event.ordinal for event in summary.events) == tuple(range(24))
    ...
    for raw_row, neutral_row in zip(batch.events, summary.events, strict=True):
        ...
        if kind == "action_rejected":
            ...
            handled.update({"agent_global_slot", "agent_anchor"})
            _assert_anchor(neutral.agent_anchor, raw.agent_anchor, keys)
        elif kind == "team_deathmatch_score_changed":
            handled.add("team_anchor")
            assert neutral.team_anchor.phase == raw.team_anchor.phase == "successor"
            assert neutral.team_anchor.team_index == raw.team_index
            assert neutral.team_anchor.team_id == raw.team_id
        elif kind == "team_deathmatch_completed":
            pass
...
def test_team_deathmatch_facts_remain_strict_global_transition_evidence() -> None:
    _, summary, _ = _summary()
    adapter = TypeAdapter(ReplayIncomingSummaryV1)
    payload = json.loads(adapter.dump_json(summary))
    score_index = summary.ordered_event_kinds.index("team_deathmatch_score_changed")
    completion_index = summary.ordered_event_kinds.index("team_deathmatch_completed")
    score = payload["events"][score_index]

    assert score["successor_score"] == (
        score["previous_score"] + score["score_increment"]
    )
    assert score["team_anchor"] == {
        "phase": "successor",
        "team_index": score["team_index"],
        "team_id": score["team_id"],
    }

    arithmetic = json.loads(json.dumps(payload))
    arithmetic["events"][score_index]["successor_score"] += 1
    with pytest.raises(ValidationError, match="exact score increment"):
        adapter.validate_json(json.dumps(arithmetic))

    team_join = json.loads(json.dumps(payload))
    team_join["events"][score_index]["team_anchor"]["team_index"] = 1
    team_join["events"][score_index]["team_anchor"]["team_id"] = 2
    with pytest.raises(ValidationError, match="join its neutral anchor"):
        adapter.validate_json(json.dumps(team_join))

    completion = json.loads(json.dumps(payload))
    completion["events"][completion_index]["completion_basis"] = "score_decision"
    with pytest.raises(ValidationError, match=r"literal_error|Input should be"):
        adapter.validate_json(json.dumps(completion))
