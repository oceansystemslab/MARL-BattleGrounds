...
test("Team Deathmatch facts remain researcher feed-only and non-spatial", async () => {
  const raw = structuredClone(fixture.presentations.replay_oracle);
  installOracleEvents(raw, [
    {
      event_kind: "team_deathmatch_score_changed",
      team_index: 0,
      team_id: 1,
      score_increment: 2,
      previous_score: 3,
    ...
    {
      event_kind: "team_deathmatch_completed",
      outcome: "team_a_win",
  ...
  assert.ok(plan);
  assert.deepEqual(
    rows.map(({ kind, vocabulary }) => [kind, vocabulary]),
    [
      ["team_deathmatch_score_changed", "event"],
      ["team_deathmatch_completed", "event"],
    ],
  );
  assert.equal(plan.events.length, 2);
  for (const event of plan.events) {
    assert.equal(event.kind, "feed_only", event.eventType);
    assert.equal(event.spatial, false, event.eventType);
    for (const forbidden of [
      "icon",
      "route",
      "paintParts",
  ...
  for (const mutate of [
    ...
    await assert.rejects(
      ...
      /Team Deathmatch score evidence/iu,
    );
  }
  ...
  for (const kind of [
    ...
    const agentFrame = await normalized(kind);
    assert.equal(
      authorizedPresentationIncomingRows(agentFrame).some(({ kind: eventKind }) =>
        eventKind.startsWith("team_deathmatch_"),
      ),
      false,
      kind,
    );
